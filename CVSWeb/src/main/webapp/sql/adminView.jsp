			...
			<table class="table table-bordered" style="width: 100%;">
				...
				<c:forEach var="memberListSort" items="${list}">
					...
					<tr>
						...
						<td align= center>
							<a href="./connectAdminPageView.jsp?id=${memberListSort.id}"  onClick="window.open(this.href, '', 'width=600, height=600'); return false;">${memberListSort.id}</a>
						</td>
						<td align="center">
						...
						<td align="center">
							<c:if
								test="${memberListSort.grade eq 'n'}">
								정상
								</c:if>
							<c:if
								test="${memberListSort.grade eq 'y'}">
								관리자
								</c:if>
							<c:if
								test="${memberListSort.grade eq 'w'}">
								경고
								</c:if>
							<c:if
								test="${memberListSort.grade eq 'b'}">
								차단
								</c:if>
						</td>	
				...
				<c:if test="${list.size() != 0}">
					...
					<c:forEach var="fb_vo" items="${list}">
						<tr>
							...
							<a href="../board/free/selectByIdx.jsp?fb_idx=${fb_vo.fb_idx}&currentPage=${freeboardList.currentPage}&job=contentView">${fb_subject} (${fb_vo.fb_commentCount})</a>
