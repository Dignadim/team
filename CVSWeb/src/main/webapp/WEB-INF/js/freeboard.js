...
function insertItem() {
	...
	if (fb_content.value.indexOf('%상품%') != -1) {
		// '%상품%' 이라는 문자열이 들어오면 모달 창을 연다.
		modal.style.display = 'block';
		fb_search();
		
		// 상품 입력이 끝나면 모달 창을 닫는다.
...
function fbInsertItem(idx) {
	let orgval = document.getElementById('fb_content').value
	document.getElementById('fb_content').value = orgval.substring(0, orgval.length-4) +
	'<br/><a href="../item/increment?idx=' + idx + '&currentPage=1&job=itemView"><img src="' + '상품 사진' + '" style="width: 200px; height: 200px;"></a><br/>'
	modalClose()
}
