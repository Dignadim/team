...
function searchFunction() {	
	...
	location.href = 'itemSearch?itemName=' + itemName + '&currentPage=1';
}
...
function itemSelectChange() {
	...
	if (selectChange == '카테고리별') {
		...
		document.getElementById('subSelectChange').innerHTML =
		'<select id="itemSubSelect" class="form-select" width="100" onchange="itemEventSelect()"><option selected>전체</option><option>원플러스원</option><option>투플러스원</option><option>카드사 할인</option><option>포인트 적립</option></select>'				
	} else if (selectChange == '가격별') {
		document.getElementById('subSelectChange').innerHTML =
