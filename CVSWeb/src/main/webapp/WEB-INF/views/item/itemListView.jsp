		...
		<c:if test="${searchList != null}">
			...
			<c:forEach var="s_vo" items="${slist}" varStatus="i">
				<span style="padding: 10px">
					<a href="itemSearch?itemName=${s_vo.keyword}&currentPage=${itemList.currentPage}">&nbsp;${s_vo.keyword}&nbsp;</a>	
				</span>
			</c:forEach>
		</c:if>
	</div>
		...
		<table class="table" style="width: 1200px; margin-left: auto; margin-right: auto; box-sizing: border-box;">
			...
			<tr>
				<td align="right" colspan="4" class="table-light">
					<c:if test="${grade == 'y'}">
						<input class="btn btn-outline-primary" type="button" value="자동 상품 등록" onclick="location.href='itemCrawling'"/>
