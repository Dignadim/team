...
//		logger.info("스케줄 매니저를 받아옴");
	...
	public String mapCrawling(RedirectAttributes rttr) {
		...
		try {
			...
			// 일부 지도 정보가 중복으로 입력됨
			// 		=> 세븐일레븐 5000여개, GS25 n개, 이마트24 n개
			//		=> GS25, 이마트24는 홈페이지 자체에 정보가 중복으로 올라와 있는 것으로 확인.
			// mapper.deleteRepeated();
	
			logger.info("모든 지도 정보 입력 완료!");
		} catch (NullPointerException e) {
			System.out.println("null pointer exception");
			rttr.addFlashAttribute("msg", "페이지에서 정보를 얻어올 수 없습니다. 다시 실행해주세요.");		
			return "map/map";			
		}
